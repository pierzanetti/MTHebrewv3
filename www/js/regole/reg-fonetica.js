angular.module('starter.reg-fonetica', [])

.factory('Fonetiche', function() {
...
{ key: '	betulah	', termine: '	בְּתוּלָה	', senza: '	בתולה	', traslitterazione: '	beṯûlāh	', significato: '	vergine	', grammatica: '	n. f.	', altro: '	cstr. בְּתוּלַת - pl. f. בְּתוּלוֹת	', id: 	108	}
  ...
  return {
    all: function() {
      return fonetiche_list;
    },
    remove: function(fonetica) {
      fonetiche_list.splice(fonetiche_list.indexOf(fonetica), 1);
    },
    get: function(foneticaId) {
      for (var i = 0; i < fonetiche_list.length; i++) {
        if (fonetiche_list[i].id === parseInt(foneticaId)) {
          return fonetiche_list[i];
        }
      }
      return null;
    }
